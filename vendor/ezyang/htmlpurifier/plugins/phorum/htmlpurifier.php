    ...
    foreach($data as $message_id => $message){
        if(isset($message['body'])) {
            ...
            if ($message_id) {
                ...
                if (isset($message['meta']['body_cache']) 
            ...
            if ($message_id  // message must be real to migrate
                && !isset($message['meta']['body_cache_serial'])
            ) {
                // perform migration
                $fake_data = array();
...
{
    include_once dirname(__FILE__).'/htmlpurifier/HTMLPurifier.auto.php';
    include dirname(__FILE__).'/init-config.php';
